    ...
    public void onApplicationEvent(ContextClosedEvent contextClosedEvent) {
        try {
            ...
            if (!gracefulShutdownWrapper.getGracefulShutdownHandler().awaitShutdown(20 * 1000)) {
                log.warn("Undertow thread pool did not shut down gracefully within 20 seconds. ");
            }
        } catch (Exception e) {
            log.warn("Undertow thread pool did not shut down gracefully within 20 seconds. ", e);
        }
    }
